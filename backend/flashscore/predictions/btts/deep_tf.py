...
def deep_model():
    ...
    merged = layers.concatenate([h2h_branch, home_away_branch, odds_branch, context_branch])
    ...
    return Model(inputs=[h2h_input, form_input, odds_input, context_input], outputs=output)
...
def train_tf_model(form_X_train, h2h_X_train, odds_X_train, context_X_train, odds_y_train):
    ...
    history = model.fit([h2h_X_train, form_X_train, odds_X_train, context_X_train], odds_y_train, epochs=30, validation_split=0.2, batch_size=32, callbacks=callbacks, verbose=0)
