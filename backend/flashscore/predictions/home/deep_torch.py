...
class DeepNN(nn.Module):
    def __init__(self):
        ...
        self.form_branch = nn.Sequential(
            nn.Conv1d(in_channels=6, out_channels=32, kernel_size=2),
            nn.ReLU(),
            nn.AdaptiveMaxPool1d(1)
        )

        self.h2h_branch = nn.Sequential(
            nn.Conv1d(in_channels=3, out_channels=32, kernel_size=2),
            nn.ReLU(),
            nn.AdaptiveMaxPool1d(1)
        )
    ...
    def forward(self, form, h2h, odds, context):
        ...
        merged = torch.cat([form_out, h2h_out, odds_out, context_out], dim=1)
...
def train_model(train_features, train_labels, test_features, test_labels,
    ...
    for epoch in range(epochs):
        ...
        for form, h2h, odds, perf, y in train_loader:
            ...
            optimizer.zero_grad()
            preds = model(form, h2h, odds, perf)
