...
def deep_model():
    ...
    game_input, game_branch = game_layer() 
    merged = layers.concatenate([odds_branch, game_branch, h2h_branch, home_away_branch])
    ...
    return Model(inputs=[odds_input, game_input, h2h_input, form_input], outputs=output)


def train_tf_model(form_X_train, h2h_X_train, odds_X_train, game_X_train, odds_y_train):
    ...
    history = model.fit([odds_X_train, game_X_train, h2h_X_train, form_X_train], odds_y_train, epochs=50, validation_split=0.2, batch_size=32, callbacks=callbacks, verbose=0)
