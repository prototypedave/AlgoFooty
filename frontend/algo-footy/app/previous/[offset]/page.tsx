...
export default function PreviousLinksPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`http://127.0.0.1:8000/gen-predictions/?day=${offset}`);
        ...
        {days.map(({ label, offset }) => {
          ...
          return (
            <li key={offset}>
              <Link
                href={href}
                className={`flex justify-between items-center w-full bg-[#002147] hover:bg-[#003366] px-4 py-2 rounded-md transition-colors
                  ${
                    isActive
                      ? "border-b-4 border-orange-red font-bold text-orange-red"
                      : "border-b-0"
                  }`}
              >
            ...
            <li key={index} className="flex bg-gradient-to-r from-[#001730] to-[#004080] w-full items-center shadow-lg p-2 h-16">
              ...
              <div className="font-semibold w-28">
                  ...
                  isNaN(Number(match.ascore))
                    ? "-" 
                    : `${parseInt(match.hscore)} - ${parseInt(match.ascore)}`}
              </div>
