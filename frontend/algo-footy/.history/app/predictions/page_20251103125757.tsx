...
interface LocalTimeFromNYProps {
  date: string; // "2025-11-02T10:00:00" → NY local wall-clock time
}

const LocalTimeFromNY: React.FC<LocalTimeFromNYProps> = ({ date }) => {
  // Split the incoming "YYYY-MM-DDTHH:mm:ss"
  const [y, m, d, h, min, s] = date.replace('T', ' ').split(/\D/).map(Number);

  // Build a string that the IANA parser understands
  const nyIso = `${y}-${String(m).padStart(2, '0')}-${String(d).padStart(
    2,
    '0'
  )} ${String(h).padStart(2, '0')}:${String(min).padStart(2, '0')}:${String(
    s ?? 0
  ).padStart(2, '0')} America/New_York`;

  const instant = new Date(nyIso); // ← this is the *correct* UTC moment

  const local = instant.toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit',
    hour12: false, // 24‑h format
  });
