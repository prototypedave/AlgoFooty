...
interface LocalTimeFromNYProps {
  date: string; // e.g., "2025-11-02T07:30:00" (New York time, no offset)
}

const LocalTimeFromNY: React.FC<LocalTimeFromNYProps> = ({ date }) => {
  // Step 1: Parse the input string as New York time
  const nyDate = new Date(date);

  // Step 2: Use Intl to interpret it in America/New_York timezone
  const nyTimeInMs = Date.UTC(
    nyDate.getFullYear(),
    nyDate.getMonth(),
    nyDate.getDate(),
    nyDate.getHours(),
  ...
  // Better: Use Intl to get correct UTC offset for that date in NY
  ...
  const nyValues = parts.reduce((acc, part) => {
    if (part.type !== 'literal') acc[part.type] = part.value;
    return acc;
  }, {} as Record<string, string>);

  const nyDateTime = new Date(
    Date.UTC(
      +nyValues.year,
      +nyValues.month - 1,
      +nyValues.day,
  ...
  // Step 3: Now format in user's LOCAL timezone
  const localTime = nyDateTime.toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit',
    hour12: false,
  });
                        ...
                        <ol className="flex flex-col w-full gap-4 pr-2">
                            {filtered.map((match, index) => (
                                <li key={index} className="flex bg-gradient-to-r from-[#001730] to-[#004080] w-full items-center shadow-lg p-2">
                                    ...
                                    <p className="font-semibold w-28">{match.prediction}</p>
                                    <div className="font-semibold w-20">{match.odds}</div>
                                    <div><LocalTimeFromNY date={match.date}/></div>
                                </li>
                            ))}
                        </ol>
                    </div>
